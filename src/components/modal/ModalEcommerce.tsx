...
const ModalEcommerce = ({ nombre, descripcion, imagen, tags, deploy, github, descripcionModal, aprendi }: ProjectModalProps) => {
    ...
    return (
        <div className="w-[80%] h-[85%] rounded-2xl bg-[#f8f8ec] px-10 shadow-lg overflow-scroll shadow-gray-800">
            ...
            <div className="mt-8">
                ...
                <p className="text-base text-gray-600">{descripcionModal}</p>
            </div>
            <div className="mt-8">
                <h3 className="text-lg font-semibold text-gray-900">Tecnologías</h3>
                <ul className="flex flex-row flex-wrap items-center justify-start gap-2 mt-4 text-sm">
                    {tags.map((tag, index) => (
            ...
            {aprendi && <div className="mt-8">
                <h3 className="mb-3 text-lg font-semibold text-gray-900">Lo que aprendí</h3>
                <p className="text-base text-gray-600">{aprendi}</p>
            </div>}
            ...
            <div className="flex flex-row gap-4 mt-8 mb-4">
                ...
                href={deploy} 
                target="_blank" 
                className="flex flex-row items-center gap-2 px-4 py-2 font-semibold text-white rounded-lg h-fit bg-violet-600 hover:bg-violet-800">
                    <ExternalLink className="w-5 h-5 stroke-2"/>Ver Proyecto
                    </a>
                <a href={github} target="_blank" className="border-[1px] border-gray-300 hover:border-gray-500 flex flex-row gap-2 px-4 items-center py-2 h-fit rounded-lg text-gray-600"><Github className="w-5 h-5 stroke-2"/>Código</a>
            </div>     
        </div>
    )
}
