        ...
        >
            <div className={`w-full max-w-6xl transition-all duration-1000 flex flex-col justify-between py-6 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className={`grid grid-cols-1 md:grid-cols-2 gap-y-16 md:gap-y-12 gap-x-6 md:gap-x-12 items-start`}>  
                    ...
                    <div className={`flex flex-col gap-6 ${!isEven ? 'md:order-2' : ''}`}>  
                        {/* Número y línea decorativa */}
                        <div 
                            className={`flex items-center gap-3 overflow-hidden transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}
                            style={{
                                transitionDelay: isVisible ? '0.2s' : '0s',
                            }}>
                            <span className="text-4xl font-light leading-none text-gray-400 dark:text-gray-600 md:text-6xl">
                                0{index + 1}
                            </span>
                        ...
                        <div className={`space-y-2 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}
                            ...
                            <h2 className="text-2xl font-bold leading-tight text-black transition-colors duration-300 dark:text-white md:text-4xl hover:text-purple-600 dark:hover:text-purple-400">
                                {proyecto.nombre}
                            </h2>
                        ...
                        <div className={`space-y-3 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}
                            ...
                            {proyecto.descripcion.map((parrafo, idx) => (
                                <p 
                                    key={idx} 
                                    className="text-sm leading-relaxed text-gray-600 transition-opacity duration-300 dark:text-gray-400 md:text-base opacity-90 hover:opacity-100"
                                >
                                    {parrafo}
                                </p>
                            ))}
                        ...
                        <div className={`space-y-2 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}
                            ...
                            <p className="text-xs font-semibold tracking-widest text-gray-500 uppercase dark:text-gray-500">{t('proyectos.stackTitle')}</p>
                    ...
                    <div className={`flex flex-col gap-4 pt-6 md:pt-12 ${!isEven ? 'md:order-1' : ''}`}>  
                        {/* Imagen: ocultar en móvil, mostrar en md+ */}
                        <div className="hidden md:block">
                            <div 
                                className={`relative w-full h-72 md:h-96 overflow-hidden rounded-xl transition-all duration-700 ${isHovered ? 'scale-105' : 'scale-100'} group ${isVisible ? 'animate-scale-in' : ''}`}
                                style={{
                        ...
                        {/* Botones debajo de la imagen (permanece visible en móvil) */}
                        <div className={`flex flex-wrap gap-2 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}
                            style={{
                            ...
                            {proyecto.linkProyecto ? (
                                ...
                                <button disabled className="px-4 py-2 text-sm font-medium text-gray-500 bg-gray-300 rounded-lg opacity-50 cursor-not-allowed dark:bg-gray-800">
                            ...
                            {proyecto.linkCodigo && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="px-4 py-2 text-sm font-medium text-black dark:text-white border border-black/50 dark:border-white-500/50 rounded-lg hover:bg-purple-500/10 hover:border-purple-600 dark:hover:border-purple-400 transition-all duration-300 flex items-center gap-2 group hover:-translate-y-0.5"
                                >
