...
const Inicio = () => {
    ...
    return (
        <section className="flex flex-col items-center justify-center h-screen bg-white dark:bg-black section">
            <h1 className="font-bold text-black dark:text-white text-7xl">{t('inicio.greeting')}</h1>
            <h1 className="font-bold text-black dark:text-white text-7xl">{t('inicio.name')}</h1>
            <h1 className="font-bold text-purple-600 dark:text-purple-400 text-7xl">{t('inicio.role')}</h1>
            <p className="w-7/12 mt-4 font-mono text-xl text-center text-gray-600 dark:text-gray-300">{t('inicio.description')}</p>
            <a 
                href="/CV-MorenaMartin-FullStack.pdf" 
                download="CV-MorenaMartin-FullStack.pdf"
                className="flex items-center gap-4 mt-6 px-6 py-1.5 border-2 border-black dark:border-white hover:bg-black dark:hover:bg-white text-black dark:text-white hover:text-white dark:hover:text-black transition-all duration-300 cursor-pointer"
            >
